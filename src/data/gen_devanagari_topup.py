"""Day 4: Devanagari (Hindi) top-up generator — script-LOCKED.

Fills the 8% Devanagari bucket with on-domain Hindi instruction-response pairs.
Llama-3.3-70B writes good Devanagari Hindi; the leak in the main pipeline was the
code-switch framing, not the model. Two stages; responses parallel.
Writes to data/raw/devanagari_topup.jsonl (separate file).
"""
...
logger = get_logger("devanagari_topup")
...
INSTR_SYS = ("You generate realistic user messages written in HINDI using DEVANAGARI script "
             "ONLY — no romanization, no English words. Output ONLY a JSON array of strings, "
             "no commentary.")
RESP_SYS = ("Reply in natural Hindi using DEVANAGARI script only — no romanization, no English. "
            "Answer the user's actual message, be helpful and concise, then stop.")
...
def instr_prompt(domain, task, register, n):
    return [
        ...
        {"role": "user", "content":
            f"Write {n} distinct {register} user messages in Hindi (Devanagari script), about "
            f"{domain}, each a {task} request. Each must be SELF-CONTAINED (any needed context "
            f"inline). Make them MAXIMALLY DIVERSE — different subtopics, no repeated template. "
            f"The user must ask for general help answerable without private accounts or "
            f"real-time data. Return a JSON array of {n} strings."},
    ]
...
def main():
    ...
    ap.add_argument("--out", default="data/raw/devanagari_topup.jsonl")
    ...
    # Stage 1: Devanagari instructions
    recs = []
    for _ in range(args.cells):
        ...
        for ins in parse_array(text):
            recs.append({"instruction": ins, "script": "devanagari", "domain": d,
                         "task": t, "register": r, "switch": "none"})
        logger.info("instructions so far: %d", len(recs))

    # Stage 2: Devanagari responses, parallel
    def work(rec):
        rec = dict(rec)
